...
public class QuestionnaireManagementService {
    ...
    public List<QuestionTaskVO> getQuestionTaskByUnitId(CurrentUser user) {
        ...
        }).collect(Collectors.toList()).stream().sorted(Comparator.comparing(QuestionTaskVO::getAnnual).reversed()).map(item -> item.setAnnual(item.getAnnual() + "年度")).collect(Collectors.toList());
    }
