...
public class ScreeningNoticeService extends BaseService<ScreeningNoticeMapper, ScreeningNotice> {
    ...
    public Boolean release(Integer id) {
        ...
        if (baseMapper.release(id) > 0) {
            List<ScreeningNoticeDeptOrg> screeningNoticeDeptOrgs = districtService.getCurrentUserDistrictTree(CurrentUserUtil.getCurrentUser()).stream().map(district -> new ScreeningNoticeDeptOrg().setScreeningNoticeId(id).setDistrictId(district.getId())).collect(Collectors.toList());
            //2. 为下属部门创建通知
