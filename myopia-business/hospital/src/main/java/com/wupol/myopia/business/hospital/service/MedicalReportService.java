...
//        medicalRecordService.finishMedicalRecord(medicalRecord);
    ...
    public MedicalReportVo getTodayLastMedicalReport(Integer hospitalId, Integer studentId) {
        MedicalReportVo report = baseMapper.getTodayLastMedicalReport(hospitalId, studentId);
        if (Objects.isNull(report)) {
            return new MedicalReportVo();
        }
        if (!CollectionUtils.isEmpty(report.getImageIdList())) {
            report.setImageUrlList(resourceFileService.getBatchResourcePath(report.getImageIdList()));
        }
        return report;
    }
