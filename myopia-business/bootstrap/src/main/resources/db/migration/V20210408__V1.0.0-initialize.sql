...
CREATE TABLE `m_district_vision_statistic`  (
  ...
  `myopia_level_early_num` int(11) DEFAULT '0' COMMENT '近视前期人数',
  `is_total` tinyint(3) UNSIGNED NOT NULL DEFAULT 0 COMMENT '是否合计数据',
...
CREATE TABLE `m_school_vision_statistic`  (
  ...
  `myopia_level_light` int(11) DEFAULT '0' COMMENT '轻度近视人数',
  `myopia_level_early` int(11) DEFAULT '0' COMMENT '近视前期',
  `myopia_level_middle` int(11) DEFAULT '0' COMMENT '中度近视人数',
  `myopia_level_high` int(11) DEFAULT '0' COMMENT '高度近视人数',
  `myopia_level_insufficient` int(11) DEFAULT '0' COMMENT '远视储备不足人数',
  `update_time` timestamp(0) NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP(0) COMMENT '视力情况--更新时间',
  `create_time` timestamp(0) NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`) USING BTREE,
