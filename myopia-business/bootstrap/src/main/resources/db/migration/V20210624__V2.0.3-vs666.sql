-- 设备上传的筛查数据
DROP TABLE IF EXISTS `device_screening_data`;
CREATE TABLE `device_screening_data`  (
  ...
  `check_mode` tinyint UNSIGNED NOT NULL DEFAULT -1 COMMENT '筛查模式. 双眼模式=0 ; 左眼模式=1; 右眼模式=2; 未知=3',
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `uni_dataid_screeningorgid_screeningtime`(`data_id`, `screening_org_id`, `screening_time`) USING BTREE COMMENT '数据id_筛查机构id_筛查时间'
);

-- 设备上传的原始数据
DROP TABLE IF EXISTS `device_src_data`;
CREATE TABLE `device_src_data`  (
  `id` int UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键id',
  `device_type` int UNSIGNED NOT NULL DEFAULT 0 COMMENT '设备类型(0=默认设备,1=vs666)',
  `data_id` int UNSIGNED NOT NULL COMMENT '数据唯一id',
