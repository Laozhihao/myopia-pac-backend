package com.wupol.myopia.business.management.cache;
...
public class DistrictCache implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        // 缓存全国行政区域-列表结构
        if (!redisUtil.hasKey(CacheKey.DISTRICT_ALL_LIST)) {
            ...
            redisTemplate.opsForHash().putAll(CacheKey.DISTRICT_ALL_LIST, districtMap);
        }

        // 缓存全国行政区域-树结构
        if (!redisUtil.hasKey(CacheKey.DISTRICT_ALL_TREE)) {
            redisUtil.set(CacheKey.DISTRICT_ALL_TREE, districtService.getWholeCountryDistrictTree());
        }

        // 缓存各省行政区域-树结构
        if (!redisUtil.hasKey(CacheKey.DISTRICT_ALL_PROVINCE_TREE)) {
            Object cacheTree = redisUtil.get(CacheKey.DISTRICT_ALL_TREE);
            ...
            redisTemplate.opsForHash().putAll(CacheKey.DISTRICT_ALL_PROVINCE_TREE, districtMap);
        }
